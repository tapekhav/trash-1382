...
void Field::generate_keys(Player& player) {
    ...
    field.at(pair1.first).at(pair1.second).set_event(new Key(&player));
    field.at(pair2.first).at(pair2.second).set_event(new Key(&player));
}

Cell Field::generate_cell(Player& player) {
    Cell cell;
    std::mt19937 rng{std::random_device{}()};
    ...
    switch(dist(rng)) {
        ...
        case 2: {
            cell.set_event(new Heal(&player));
            break;
        }
        case 3: {
            cell.set_event(new Box(&player));
            break;
        }
        case 4:
            cell.set_event(new Trap(&player));
            break;
        case 5: {
            cell.set_event(new NewLevel(this));
            break;
        }
        default:
            break;
...
void Field::make_field(Player& player) {
    ...
    for(int i = 0; i != height; ++i) {
        int val = i == 0 ? 1 : 0;
        for(int j = val; j != width; ++j) {
            field.at(i).at(j) = generate_cell(player);
        }
    }
    generate_keys(player);
    this->notify();
}
