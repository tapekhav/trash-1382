...
Field::Field(std::pair<int, int> fieldSize, std::pair<int, int> playerPosition):fieldSize(fieldSize),playerPosition(playerPosition){
    setField();
};
...
void Field::setField(){
    ...
    std::cout << "Exit h = " << exitPosition.second << " exit w = " << exitPosition.first <<'\n';
    for(int h = 0; h < fieldSize.second; ++h){
        field.emplace_back();
        for(int w = 0; w < fieldSize.first; ++w){
            ...
            if(w == 9 and h == 9){
                auto openChest = new EventPlayerOpenChest(20);
